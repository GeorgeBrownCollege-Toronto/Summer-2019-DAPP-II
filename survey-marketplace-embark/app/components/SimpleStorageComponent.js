...
class SimpleStorageComponent extends React.Component {
  constructor(props) {
    super(props);
  ...
  async setValue(e) {
    e.preventDefault();
    // const acc = await ethereum.selectedAddress;
    // EmbarkJS.Blockchain.Providers.web3.setDefaultAccount(acc);
    var value = parseInt(this.state.valueSet, 10);
    const gasLimit = await SimpleStorage.methods.set(value).estimateGas();
    const from = ethereum.selectedAddress;
    const tx = await SimpleStorage.methods.set(value).send({ from, gasLimit });
    console.log("--", tx);
      ...
      <React.Fragment>
        ...
        <Form onKeyDown={e => this.checkEnter(e, this.setValue)}>
          <FormGroup className="inline-input-btn">
            ...
            />
            <Button color="primary" onClick={e => this.setValue(e)}>
              Set Value
            </Button>
            <FormText color="muted">
              Once you set the value, the transaction will need to be mined and
              then the value will be updated on the blockchain.
            </FormText>
          </FormGroup>
        </Form>
        ...
        <Form>
          <FormGroup>
            ...
            <FormText color="muted">
              Click the button to get the current value. The initial value is
              100.
            </FormText>
